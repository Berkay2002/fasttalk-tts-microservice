...
Provides streaming text-to-speech synthesis using RealtimeTTS with multiple
engine support (Kokoro, Coqui, Orpheus) for low-latency audio generation.
"""
...
# Default configuration constants (matching backend-orchestration)
START_ENGINE = "kokoro"
...
class AudioProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            kokoro_voice: The voice to use for Kokoro engine (used only if engine is "kokoro").
        """
        ...
        if engine == "coqui":
            ...
            logger.info(f"🎤 Initializing Kokoro engine with voice: {kokoro_voice}")
            self.engine = KokoroEngine(
                voice=kokoro_voice,
                default_speed=1.26,
                debug=True,  # Enable debug mode to see GPU/CPU usage
            )
            logger.info("🎤 Kokoro engine initialized (GPU used if available)")
        elif engine == "orpheus":
    ...
    def _prewarm_engine(self) -> None:
        """Prewarm the TTS engine with a simple synthesis."""
    ...
    def _measure_ttfa(self) -> None:
        """Measure Time To First Audio (TTFA) for the engine."""
