...
def extract_chapters_from_epub(epub_file, debug=False):
    ...
    def _has_chapter_class(value: Optional[str]) -> bool:
        return bool(value and 'chapter' in value.lower())
    ...
    def _is_chapter_heading(tag: Tag) -> bool:
        ...
        if not isinstance(text, str):
            return False
        lowered = text.lower()
        return 'chapter' in lowered or 'book' in lowered
        ...
        def print_toc(items, depth=0):
            for item in items:
                ...
                if isinstance(item, tuple):
                    ...
                    link = cast(epub.Link, item)
                    print(f"{indent}• {link.title} -> {link.href}")
        print_toc(book.toc)
    ...
    def process_toc_items(items, depth=0):
        ...
        for i, item in enumerate(items):
            if isinstance(item, tuple):
                ...
                link = cast(epub.Link, item)
                if debug:
                    print(f"{'  ' * depth}Processing link: {link.title} -> {link.href}")
                
                # Skip if title suggests it's front matter
                if (link.title.lower() in ['copy', 'copyright', 'title page', 'cover'] or
                    link.title.lower().startswith('by')):
                    continue
                ...
                if doc:
                    ...
                    if not fragment_id:
                        ...
                        if isinstance(next_item, epub.Link):
                            next_link = cast(epub.Link, next_item)
                            href_value = next_link.href or ""
                            next_href_parts = href_value.split('#')
                    ...
                    if text_content:
                        ...
                        if debug:
                            print(f"{'  ' * depth}Added chapter: {link.title}")
    ...
    if not chapters:
        ...
        for doc in docs:
            ...
            chapter_divs = soup.find_all(['h1', 'h2', 'h3'], class_=_has_chapter_class)
            if not chapter_divs:
                chapter_divs = soup.find_all(_is_chapter_heading)
...
def convert_text_to_audio(input_file, output_file=None, voice=None, speed=1.0, lang="en-us", 
    ...
    if input_file.endswith('.epub'):
        ...
        if not chapters:
            ...
            if split_output:
                split_output_dir = cast(str, split_output)
                ...
                for chapter_num, chapter in enumerate(chapters, 1):
                    chapter_dir = os.path.join(split_output_dir, f"chapter_{chapter_num:03d}")
    ...
    if stream:
        ...
        if split_output:
            ...
            print(f"\nCreated audio files for {len(chapters)} chapters in {split_output_dir}/")
        else:
...
# --- Wrapper to handle raw text (simpler than full file-based function) ---
def convert_text_to_audio_text(
    text, 
    output_file=None, 
    voice=None, 
    speed=1.0, 
    ...
    if voice:
        ...
        voice = "af_sarah"  # fallback

    if not output_file:
        output_dir = "output"
        os.makedirs(output_dir, exist_ok=True)
        output_file = os.path.join(output_dir, f"{uuid.uuid4()}.{format}")
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        if samples is not None:
            if sample_rate is None:
                sample_rate = sr
            all_samples.extend(samples)
        yield {"progress": i / len(chunks)}  # report progress as float (0-1)

    if all_samples:
        sf.write(output_file, all_samples, sample_rate)
        yield {"done": True, "file": output_file, "format": format}
    else:
...
async def tts_handler(websocket):
    ...
    {"status": "progress", "progress": 0.3}
    {"status": "progress", "progress": 0.7}
    {"status": "ok", "file": "output/xxxx.wav", "format": "wav"}
    or
        ...
        try:
            ...
            if not text:
                try:
                    ...
                    print("Connection closed when sending 'No text provided' error")
                    break  # or return to stop handling this connection
                continue
            ...
            # Run conversion generator
            async def run_conversion():
                try:
                    for update in convert_text_to_audio_text(
                        text=text,
                        output_file=None,
                        voice=voice,
                        speed=speed,
                        ...
                        if "progress" in update:
                            try:
                                ...
                                print("Connection closed during sending progress update")
                                return  # stop conversion on connection lost
                        elif "done" in update:
                            try:
                                await websocket.send(json.dumps({
                                    "status": "ok",
                                ...
                                print("Connection closed during sending done message")
                            return  # after done, exit run_conversion
                except Exception as e:
                    try:
                        await websocket.send(json.dumps({
                            "status": "error",
                        ...
                        print("Connection closed during sending error message")
            ...
            try:
                ...
                print("Connection closed during sending error message")
            break  # optionally break if you want to stop processing on errors



async def main_ws(host="0.0.0.0", port=8000):
    async with websockets.serve(tts_handler, host, port):
        print(f"WebSocket server running at ws://{host}:{port}")
        await asyncio.Future()  # run forever

if __name__ == "__main__":
    import sys
    print(f"WebSocket server  at ws://{sys.argv}")
    if "--websocket" in sys.argv:
        print("WebSocket")
        asyncio.run(main_ws())
   # else:

       # main()  # your existing CLI entry point
